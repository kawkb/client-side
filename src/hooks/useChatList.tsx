...
function createRandomDM(): DMs {
  let user: User = new User(
    ...
    faker.name.fullName(),
    faker.image.avatar(),
    faker.helpers.arrayElement([Status.ONLINE, Status.OFFLINE, Status.INGAME]),
...
const useChatList = create(
  combine(
    {
      items: [] as Array<Channel | DMs>,
      activeItem: null as Channel | DMs | null,
    },
    (set) => ({
      add: (item: Channel | DMs) =>
        set((state) => ({
          items: [...state.items, item] as Array<Channel | DMs>,
        })),
      remove: (itemId: string) =>
        set((state) => ({
          items: state.items.filter((i: Channel | DMs) => {
            if (i.id !== itemId) return i as Channel | DMs;
          }) as Array<Channel | DMs>,
        })),
      setActiveItem: (itemId: string) =>
        set((state) => ({
          activeItem: state.items.find(
            (i: Channel | DMs) => i.id === itemId
          ) as Channel | DMs,
        })),
      setItems: (myitems: Array<Channel | DMs>) =>
        set((state) => ({ items: myitems as Array<Channel | DMs> })),
      setMessages: (messages: Array<ChannelMsg | DMsMsg>) =>
        set((state) => ({
          activeItem: { ...state.activeItem, messages: messages } as
            | Channel
            | DMs,
        })),
    })
